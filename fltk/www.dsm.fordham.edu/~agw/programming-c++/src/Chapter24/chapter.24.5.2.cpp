...
int main()
{
    ...
    // Matrix<int> b = apply(abs,a);
    // Matrix<int> b = apply<int (*)(int), Matrix<int,1>>(abs,a);
    // Matrix<int> b = apply(static_cast<int (*)(int)>(abs),a);
    // using Int_func = std::function<int(int)>;
    using Int_func = int(*)(int);
    Matrix<int> b = apply(static_cast<Int_func>(abs),a);
